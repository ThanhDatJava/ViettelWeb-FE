...
const Package5GUpdate = (props: IProps) => {
  ...
  useEffect(() => {
    if (dataUpdate) {
      ...
      form.setFieldsValue({
        ...
        mybox: dataUpdate.mybox,
        offline: dataUpdate.offline,
        internalnetwork: dataUpdate.internalnetwork,
      });
    }
  }, [dataUpdate]);
  ...
  const onFinish = async (values: any) => {
    if (dataUpdate) {
      const { goicuoc, gia, tinhnang, type, mybox, offline, internalnetwork } =
        values;
